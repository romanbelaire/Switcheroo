...
public class playerController : MonoBehaviour {
    ...
    public int direction; //direction int representing 4 cardinal directions from top clockwise
    ...
    {
        //movement penalty for agent
        GameData.agent.AddReward(-0.01f);
        //moves the player
        int x = 0;
        int y = 0;
    ...
    protected void OnTriggerEnter2D(Collider2D other)
    {
        ...
        {
            GameData.Grid[(int)Mathf.Round(transform.position.x) + 4, (int)Mathf.Round(transform.position.y) + 4] = null;//remove from grid
            other.gameObject.SetActive(false);
            if(other.gameObject.GetComponent<SpriteRenderer>().color == GameData.goodColor)
            {
                GameData.score += 1;
                goodCount += 1;
                GameData.agent.AddReward(1.0f);//add reward to agent
            }
            else
            {
                GameData.score -= 1;
                GameData.agent.AddReward(-1.0f);//penalize agent
            }
        }
        if(goodCount == 5)
        {
            GameData.agent.EndEpisode();
            if (GameData.level < 3)
            {
